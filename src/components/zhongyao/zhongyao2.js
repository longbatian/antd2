  ...
  componentDidMount(){
    ...
    //  本周销量排行
        $.ajax({
                ...
                type: "post",
                data: {
                  "username":username,"token":token,"type":1
    ...
    //  总销量排行
        $.ajax({
                ...
                type: "post",
                data: {
                "username":username,"token":token,"type":2
             },
                dataType: "json",
                success: function(data){
                      if(data.data.length==0){
            ...
            <ul className='zhongyao_rank_ul' id='cd'>
              {
                this.state.rank1.map(function (item,i){
                  ...
                  <li key={item.id+'zhY2rank1'} className='relative' data={item.id}
                      onClick={(e)=>{this.xiangqing1(e,item.id)}}>
                      <div className='zhongyao_rank_con_zheng floatleft' ><img src={this.state.lujin+item.image} className='width69' alt=""/></div>
              ...
              <li key={item.id+'zhYrank2'} className='zhongyao_rank_ul_li'
                  onMouseOver={(e)=>{this.rank(e)}}
                  onMouseOut={(e)=>{this.rank1(e)}} data={item.id}
                  onClick={(e)=>{this.xiangqing1(e,item.id)}}>
                {/*<Link to={'/Shangpinxiangqing?&id='+item.id}>*/}
                  <div className='zhongyao_rank_ul_yuan floatleft'>{i+4}</div>
                  <img src={this.state.lujin+item.image} alt="" className='zhongyao_rank_ul_img_current display floatleft'/>
          ...
          <ul className='zhongyao_rank_ul'>
            {
              this.state.rank3.map(function (item,i){
                return(
                  <li className='relative' key={item.id+'zhYrank3'} data={item.id} onClick={(e)=>{this.xiangqing1(e,item.id)}}>
                    <div className='zhongyao_rank_con_zheng floatleft'><img src={this.state.lujin+item.image} className='width69'  alt=""/></div>
                    <div className='zhongyao_rank_ul_rank'>{i+1}</div>
                    <span className='floatleft hid width110'>{item.title}</span>
                    <span className='floatleft hid width110'>精选{item.sku}</span>
              ...
              this.state.rank4.map(function (item,i){
                return(
                  <li key={item.id+'zhYrank4'} className='zhongyao_rank_ul_li'
                      onMouseOver={(e)=>{this.rank(e)}} onMouseOut={(e)=>{this.rank1(e)}} key={i} data={item.id}
                      onClick={(e)=>{this.xiangqing1(e,item.id)}} >
                    <div className='zhongyao_rank_ul_yuan floatleft'>{i+4}</div>
                    <img src={this.state.lujin+item.image} alt="" className='zhongyao_rank_ul_img_current display floatleft'/>
                    <span className='floatleft width165 hid'>{item.title}</span>
